...
let include_module = (env, sod) => {
  let filepath = Some(sod.pimp_path.txt);
  let mod_name = sod.pimp_name.txt;
  let path =
    Typetexp.lookup_module(~load=true, env, sod.pimp_loc, mod_name, filepath);
    ...
    List.fold_right(
      (item, (sigs, stmts)) =>
        ...
        | PExposeModule({name: {txt: IdentName(name)}, alias, loc}) =>
          let (mod_id, mod_decl) =
            Typetexp.find_module(env, loc, IdentName(name), None);
          let sig_ =
            switch (alias) {
            | Some({txt: IdentName(alias)}) =>
...
let use_implicit_module = (m, env) => {
  ...
  let include_desc = {
    ...
    pimp_name: Location.mknoloc(ident),
    pimp_loc: loc,
  };

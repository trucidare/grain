...
describe("exceptions", ({test, testSkip}) => {
  ...
  assertRunError(
    ...
    {|include "exception" as Exception; exception HorribleError; Exception.registerPrinter(e => match (e) { HorribleError => Some("Spooky error"), _ => None }); let _ = throw HorribleError|},
    "Spooky error",
  );
  assertRunError(
    "exception_register_2",
    {|
    include "exception" as Exception
